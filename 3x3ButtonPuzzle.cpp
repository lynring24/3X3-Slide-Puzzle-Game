...
public:	
	...
	DECLARE_MESSAGE_MAP()
	void swap(int idc); //바꾸기
	int naver(int, int);//이웃 확인
	void complete(); // 담 확인
	void Success();
};
...
int CPuzzleWindow::naver(int idc, int n=nine-100){//이웃 체크 9가 들어있는 칸을 (x,y,x+100,y+100)
	if((r[idc].left==r[n].left-100) && (r[idc].top==r[n].top))//상 : x-100,y,x,y+100
		return 1;
	else if ((r[idc].top==r[n].top) && (r[n].left+100==r[idc].left))//우 : x+100,y,x+200,y+100
		return 1;
	else if ((r[idc].left==r[n].left) && (r[idc].top==r[n].top-100))//상 : x, y-100, x+100,y
		return 1;
	else if ((r[idc].left==r[n].left) && (r[idc].top==r[n].top+100))//하 : x,y+100,x+100,y+200
		return 1;
	else 
		return 0;
}
